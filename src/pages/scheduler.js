...
export const URLifybirthday = (str) => str
    .split(' ')
...
const setQueryParam = (value) => {
  return `?${new URLSearchParams({ birthday: value }).toString().toLowerCase()}`;
}
...
const getMsgs = (schedule) => {
    if (!schedule) return []; 
    const [numberOfFeedings, mergeCount] = getDailyFeedingsAndMergeCount(schedule);
...
const Scheduler = ({ data }) => {
  ...
  const renderMessages = () => {
    if (schedule) {
      return getMsgs(schedule)
        .map(msg =>
          <li className="p-10 border-2 mx-10 w-full rounded my-2 text-center">{msg}</li>
        )
    }
    return [];
  }
  ...
  return (
    <SEO siteMetadata={siteMetadata}>
      ...
      <input type="text" value={parseUrlbirthday(birthday)} placeholder="Enter birthday" className="border border-2 text-xl border-gray-400 rounded-full p-5 flex mx-auto my-10 w-64 outline-none" onChange={handleSetBirthday} onKeyDown={handleKeyChange} />
