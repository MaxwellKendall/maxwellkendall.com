...
export const URLifySearchTerm = (str) =>
  str.split(' ').map((s) =>
    s
      .split('')
...
const Blog = ({ data }) => {
  ...
  return (
    <SEO siteMetadata={{ title: 'Blog', description: 'test' }}>
      <Header izOffHrs={izOffHrs} />
      {/* <input
        type="text"
        value={parseUrlSearchTerm(searchTerm)}
      ...
      {/* <ul className="blog-list flex flex-wrap mx-auto justify-center">
        {posts
          ...
          .filter(({ node: post }) => {
            ...
            const headers = post.headings.map(({ value }) =>
              value.toLowerCase()
            );
            return (
              tags?.some((tag) =>
                tag.includes(parseUrlSearchTerm(searchTerm))
              ) ||
          ...
          .map(({ node: post }) => {
            ...
            return (
              <li
                className="blog-post__preview flex w-full p-10 bg-white mx-8 md:w-1/2"
                key={post.id}
              >
                <Link to={post.fields.slug} className="flex flex-col">
                  ...
                  <p className="text-xl py-4 w-full">{post.excerpt}</p>
                  <ul className="flex">
                    {post.frontmatter.tags &&
                      post.frontmatter.tags
                        .split(',')
                        ...
                        .map((t) => {
                          ...
                          return (
                            <li
                              className="px-4 py-2 border-2 rounded mx-2"
